...
use kornia_gpu::{kernels, pool::GpuImagePool, GpuAllocator, ImageExt};
    ...
    fn from_env() -> Self {
        Self {
            ...
            width: std::env::var("CAM_WIDTH")
                ...
                .and_then(|v| v.parse().ok())
                .unwrap_or(1280),
            homography: [1.2, 0.05, -120.0, -0.03, 1.15, -90.0, 0.0001, 0.0002, 1.0],
        }
    }
}
...
impl BevPipeline {
    fn new(cfg: &BevConfig) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        println!(
            "[bev_node] GPU ready - {}×{} pool allocated",
    ...
    fn process_frame(
        ...
        println!(
            "[bev_node] upload={:.1}ms  kernel={:.1}ms  download={:.1}ms",
